...
export function NewThreadModal({ isOpen, onClose, onSubmit }: NewThreadModalProps) {
  ...
  const renderBody = () => (
    <>
      <div className={isDesktop ? "px-6 py-5 space-y-4" : "py-5 space-y-4"}>
        <div>
      ...
      <div className={`flex gap-3 py-4 border-t border-outline-variant/20 ${isDesktop ? "px-6" : ""}`}>
  ...
  return (
    <BottomSheet open onClose={onClose} title="Iniciar Debate">
      {renderBody()}
    </BottomSheet>
  );
}