...
import { MaterialIcon, BottomSheet } from "@/components/ui";
...
export default function EventCensusModal({ eventId, eventTitle, open, onClose }: EventCensusModalProps) {
  ...
  if (isDesktop) {
    return (
      ...
      >
        <div
          className="w-full max-w-md bg-surface-container-low rounded-3xl border border-outline-variant/20 shadow-2xl max-h-[80vh] flex flex-col"
  ...
  return (
    <BottomSheet open onClose={onClose} title="Asistentes" showTitle={false}>
      {renderHeader()}
      {renderList()}
    </BottomSheet>
  );
